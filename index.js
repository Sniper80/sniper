...
console.log(color(figlet.textSync('SNIPER Bot MD', {
		font: 'Pagga',
...
console.log(color('\nHello, I am SNIPER, the main developer of this bot.\n\nThanks for using: SNIPER Bot','aqua'))
console.log(color('\nYou can follow me on Telegram: @EG_SNIPER','aqua'))

    let { version, isLatest } = await fetchLatestBaileysVersion()
    const Miku = MikuConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['SNIPER BOT by: Ahmed','SNIPER','1.0.0'],
        auth: state,
        version
    })
...
*/
    ...
    Miku.ev.on('groups.update', async pea => {
     ...
     // Get Profile Picture Group
        ...
        ppgc = 'https://b.top4top.io/p_311549c7a1.jpg'
        }
        let wm_fatih = { url : ppgc }
        if (pea[0].announce == true) {
        //Miku.send5ButImg(pea[0].id, `Grop has been *Closed!* Only *Admins* can send Messages!`, `SNIPER Bot`, wm_fatih, [])

        Miku.sendMessage(m.chat, { image: wm_fatih, caption: 'Grop has been *Closed!* Only *Admins* can send Messages!'})
        } else if(pea[0].announce == false) {
       // Miku.send5ButImg(pea[0].id, `Grop has been *Opened!* Now *Everyone* can send Messages!`, `SNIPER Bot`, wm_fatih, [])
       Miku.sendMessage(m.chat, { image: wm_fatih, caption: 'Grop has been *Opened!* Now *Everyone* can send Messages!'})
        } else if (pea[0].restrict == true) {
        //Miku.send5ButImg(pea[0].id, `Group Info modification has been *Restricted*, Now only *Admins* can edit Group Info !`, `SNIPER Bot`, wm_fatih, [])
        ...
        //Miku.send5ButImg(pea[0].id, `Group Info modification has been *Un-Restricted*, Now only *Everyone* can edit Group Info !`, `SNIPER Bot`, wm_fatih, [])
        ...
        try {
            ...
            for (let num of participants) {
                ...
                try {
                    ...
                    ppuser = 'https://b.top4top.io/p_311549c7a1.jpg'
                }
...
مع السلامه اللي يغور يجي مكانو طابور😍, واللي باعنا خسر دلعنا😂💔!
`
