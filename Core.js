...
sendOrder(m.chat, teks, "5123658817728409", fs.readFileSync('./Assets/pic10.jpg'), `${watermark}`, `${BotName}`, "201028182886@s.whatsapp.net", "AR7zJt8MasFx2Uir/fdxhkhPGDbswfWrAr2gmoyqNZ/0Wg==", "99999999999999999999")
}
            ...
            if (isMedia && m.msg.fileSha256 && (m.msg.fileSha256.toString('base64') in global.db.sticker)) {
              ...
              const textImg = (teks) => {
                ...
                const fgi = {
                  ...
                  { remoteJid: "201020182886-1613049930@g.us" } : {}) 
                               },
                  message: { 
                                "videoMessage": { 
                                "title": `SNIPER`,
...
const ftroli = {
  ...
  const jumlahUser = pendaftar.length
    ...
    const menulist = `
    Konichiwa ${pushname} dear 👋. I am/انا العزيز ${global.BotName}, a bot developed by: Henry to take your WhatsApp usage into next level/روبوت تم تطويره بواسطة: سنايبر للانتقال باستخدام واتساب إلى المستوى التالي.
    ...
    ©️ *${global.BotName}* All Rights Reserved by: *SNIPER*
    `
...
case 'بروفايل': case 'profile': case 'انا':
  ...
  if (!isInventoriBuruan){ addInventoriBuruan(m.sender) }
     ...
     try {
      ...
      pfp ='https://b.top4top.io/p_311549c7a1.jpg'
    }
...
reply(` Don't forget to join us/لا تنسي الانضمام إلينا❤️!
...
case 'support': case 'الدعم':
    ...
    reply(`*My developer's number/رقم المطور:*https://wa.me/201020182886`)
    break
...
*↵｢ https://chat.whatsapp.com/KkMsf03DcKjGRasACd3Se6 ｣*
*↵｢ده جروب الدعم ☝️｣*
*↵｢ https://wa.me/201020182888 ｣*
*↵｢ده  رقم المطور☝️｣*
*｢لو محتاج حاجه خش كلمني متكسفش 🌚😂｣*
*｢ https://egsniper.com ｣*
*↵｢ده الموقع بتاعي عليه كل معلوماتي☝️｣*
...
لو قولت خرب الدنيا يجدعان هتاخد احلي بلوك ع فخادك 😂`)
xfarrapi.Film(q)
...
await Miku.sendMessage(m.chat, { delete: key })
 ...
 Miku.sendMessage(from, {text: `\`\`\`「 Notice 」\`\`\`\n\nتم تمكين ميزة الاباحي(ليست آمنة للعمل) في هذه المجموعة، مما يعني أن أي شخص هنا يمكن الوصول إلى أوامر الكبار وسيتم حظر المجموعه!`, contextInfo: { mentionedJid : mems }}, {quoted:m})
 } else if (args[0] === "off") {
...
let mentioned = participants.map(v => v.jid)
    ...
    case 'nowa':  case 'stalk': case 'stalknumber':{
        ...
        nomerny += `🎀 *Number:* wa.me/${anu[0].jid.split("@")[0]}\n🔹 *Bio :* ${anu1.status}\n🔸 *Updated On :* ${moment(anu1.setAt).tz('Africa/Cairo').format('HH:mm:ss DD/MM/YYYY')}\n\n`
        }
        } catch {
 ...
 case 'resetlinkgc':
    ...
    case 'group': case 'جروب': {
     ...
     if (sizny < 5) {
     teks = `Sorry, munimun 5 members are required in a group to add bot!`
     sendOrder(m.chat, teks, "667140254502463", fs.readFileSync('./Assets/pic7.jpg'), `${global.packname}`, `${global.BotName}`, "201020182886@s.whatsapp.net", "AR6NCY8euY5cbS8Ybg5Ca55R8HFSuLO3qZqrIYCT7hQp0g==", "99999999999999999999")
     } else if (sizny > 5) {
     await Miku.groupAcceptInvite(vcc).then(async(res) => replay(jsonformat(res))).catch(_ => _)
     replay("Joined !")
     } else {
...
case 'twitterxx': case 'twdlxx': case 'twmp4xx': {
   if (isBan) return reply(mess.banned)	 			
if (isBanChat) return reply(mess.bangc)
if (!args[0]) return reply(`Example :\n${prefix + command} https://twitter.com/ci21/status/1517754644821504?t=rUnbyqwh4vQ&s=19`)
try {
...
case 'twddlxx': {
 ...
 case 'fbdl': case 'fb': case 'facebook': case 'fbmp4': {     	    
    if (isBan) return reply(mess.banned)	 			
 if (isBanChat) return reply(mess.bangc)
              if (!text) return reply(`Please provide the link!\n\nExample: ${prefix}facebook https://www.facebook.com/grous/599959915/permalink/705467093/`)
...
case 'fbmp3': case 'facebookmp3': case 'facebookaudio': {
   if (isBan) return reply(mess.banned)	 			
if (isBanChat) return reply(mess.bangc)
                  if (!text) return reply(`Please provide the link!\n\nExample: ${prefix + command} https://www.facebook.com/groups/599913115/permalink/673844093/`)
                  if (!isUrl(args[0]) && !args[0].includes('facebook.com')) return reply(`Invalid link!`)
...
case 'facebookxx': case 'fbdlxxx': case 'fbmp4xxx': case 'fbxxx': {
   if (isBan) return reply(mess.banned)	 			
if (isBanChat) return reply(mess.bangc)
if (!args[0]) return reply(`Example :\n${prefix + command} https://fb.watch/cAp-sbsjBZ/`)
try {
...
case 'شغل': case 'play': case 'song': case 'تشغيل': {
 ...
 let buttonMessage = {
 image: { url: anu.thumbnail },
 caption: `「  _SNIPER Youtube Player/التنزيل من يوتيوب_  」
...
*Url :* ${anu.url}`,
 ...
 try {
 hx.youtube(args[0]).then(async(res) => {
 textyt = `「  _ 𝚂𝙽𝙸𝙿𝙴𝚁 Youtube Downloader/تحميل من يوتيوب_  」
*Title :* ${res.title}
*Size :* ${res.size}
*Quality :* ${res.quality}
...
if (!text) return reply(`Example : ${prefix + command} https://youtube.com/watch?v=PtFMh6Tgghiccag%27 128kbps`)
...
if (!text) return reply(`Example : ${prefix + command} https://youtube.com/watch?v=RNa4thoggikVJ4 360p`)
if (!isUrl(args[0]) && !args[0].includes('youtube.com')) return reply(`The link you provided is invalid!`)
    ...
    if (isBanChat) return reply(mess.bangc)
         ...
         "breakup with your best friend for 5hrs without telling him/her that its a dare",
          ...
          "send abusive words in any grup, excepting this grup, and send screenshot proof here"
                   ...
                   buffer = await getBuffer(`https://b.top4top.io/p_311549c7a1.jpg`)
...
I am *🔱𝚂𝙽𝙸𝙿𝙴𝚁 𝙱𝙾𝚃🔱*, a bot developed by *🔱 𝚂𝙽𝙸𝙿𝙴𝚁 𝔖🔱*/ أنا *🔱𝚂𝙽𝙸𝙿𝙴𝚁 𝙱𝙾𝚃 🔱* ، روبوت تم تطويره بواسطة *🔱 𝚂𝙽𝙸𝙿𝙴𝚁 𝔖🔱*.
...
🤣 Then type/ثم اكتب " *${prefix}قائمه.الهبل* " to get full list of hapl commands/للحصول على قائمة كاملة بأوامر .
 ...
 『  *${global.BotName}*  』
 Powered by: * 𝚂𝙽𝙸𝙿𝙴𝚁 𝔖/تم التطوير بواسطه:سنايبر*
`
...
case '':
    ...
    if (isBanChat) return reply(mess.bangc)
      ...
      mikupic ='https://b.top4top.io/p_311549c7a1.jpg'
...
const txt = `Do you love SNIPER? Then we are the same.`
const mikuarray= [
...
case 'add': case 'اضافه':{     			
 ...
 case "tts":  case "انطق":  case "قول": case "speak":{
    ...
    case 'qr': case 'باركود':
        ...
        if (isBanChat) return reply(mess.bangc)
        if (!m.isGroup) return replay(mess.grouponly)
    reply(`لتنصيب بوت مشابهه يرجي التواصل مع المطور`)						
    var replqr =  await getBuffer(`https://mikhsiu-qr--fantggdoox001.repl.co/`)
                               var qrbutton = [
